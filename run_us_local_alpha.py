...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="US Local Pipeline Runner (Alpha Fork)")
...
def train(panel: pd.DataFrame, benchmark_close: pd.Series,
    ...
    if train_end is not None:
        ...
        if _zone_feat_cols:
            _need = [c for c in ("open", "high", "low", "close", "volume",
                                 f"{FEATURE_PREFIX}weekly_trend",
                                 f"{FEATURE_PREFIX}monthly_trend",
                                 f"{FEATURE_PREFIX}quarterly_trend",
                                 f"{FEATURE_PREFIX}yearly_trend")
    ...
    if n_trials > 0:
        ...
        study_name = f"us_local_alpha_hpo_{mode}"
...
def score_and_rank(panel: pd.DataFrame, ensemble, final_features: List[str],
    ...
    shap_img_path = REPORTS_DIR / "shap_global_us_local_alpha.png"
...
def main() -> None:
    ...
    from pathlib import Path
    _log_dir = Path("artefacts/us_local_alpha/logs")
