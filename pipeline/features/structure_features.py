...
Toggleable via MarketConfig.use_structure_features (default False). When off,
engineer.py never calls this module and behaviour is identical to baseline.
...
# ── Production helper: model-ready, finite, prefixed columns ───────────────────
# bars_since_* are inf before the first event; saturate at one trading year so
# the columns are finite for winsorisation / schema histograms / LightGBM.
# "More than a year since the last event" is a sensible recency ceiling and
# matches the expiry philosophy already used in ict_features / zone_features.
BARS_SINCE_CAP = 252.0

# The model-facing columns produced by compute_multiscale_structure_features,
# in a stable order (excludes raw OHLCV which df.copy() carries through).
STRUCTURE_OUTPUT_COLS = (
    [f"major_{x}" for x in (
        "trend_state", "bos_flag", "choch_flag", "bsl_swept", "ssl_swept",
        "bars_since_bos", "bars_since_choch", "bars_since_bsl_sweep",
        "bars_since_ssl_sweep", "level_dist_atr")]
    + [f"internal_{x}" for x in (
        "trend_state", "bos_flag", "choch_flag", "bsl_swept", "ssl_swept",
        "bars_since_bos", "bars_since_choch", "bars_since_bsl_sweep",
        "bars_since_ssl_sweep", "level_dist_atr")]
    + ["structure_alignment"]
...
def structure_feature_frame(
    ...
    """
    Engineer-facing wrapper: compute multi-scale structure features for one
    ticker's daily OHLCV and return ONLY the model columns — finite (inf in
    bars_since_* capped at BARS_SINCE_CAP), float32, optionally prefixed.

    Parameters
    ----------
    ohlcv       : single-ticker daily OHLCV, date-indexed, lower-case columns.
    cutoff_date : per-fold causal cutoff (pass fold.train_end). None = full build.
    prefix      : e.g. "features_" so columns are picked up by the model.

    Returns
    -------
    DataFrame indexed like ohlcv with exactly the prefixed structure columns.
    """
    out = compute_multiscale_structure_features(
        ohlcv, major_swing_length, minor_swing_length, cutoff_date=cutoff_date
    )
    cols = [c for c in STRUCTURE_OUTPUT_COLS if c in out.columns]
    frame = out[cols].copy()
    for c in cols:
        if c.startswith(("major_bars_since", "internal_bars_since")):
            frame[c] = frame[c].replace([np.inf, -np.inf], BARS_SINCE_CAP).clip(upper=BARS_SINCE_CAP)
        frame[c] = frame[c].astype(np.float32)
    if prefix:
        frame = frame.rename(columns={c: f"{prefix}{c}" for c in cols})
    return frame
