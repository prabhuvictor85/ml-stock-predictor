"""
run_nse_local.py — End-to-end NSE pipeline using LOCAL CSV stock data.

Reads:
  - Stock list  : C:/Victor/Learning_charts/stock_lists/constituentsi.csv
                  Columns: Symbol1, Symbol  (Symbol = Yahoo ticker e.g. 360ONE.NS)
  - Daily data  : C:/Victor/Learning_charts/stock_data/{TICKER}-1d.csv
...
Steps:
  ...
  4.  Purged walk-forward CV (auto-computed folds based on years of data — same
      formula as run_sp500_local.py; typically ~14 folds against the full
      pre-2010-to-present NSE history)
...
Usage:
    python run_nse_local.py
    python run_nse_local.py --top_n 15 --weighting inverse_vol
    python run_nse_local.py --skip_train  (load existing artefacts and re-score)
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--train_start", type=str, default="2010-01-01",
                   help="Earliest date included in the training panel (default: 2010-01-01). "
                        ...
                        "so all folds land in the meaningful data window. "
...
def load_benchmark(data_dir: Path) -> pd.Series:
    """Load benchmark close. Try local file first, then yfinance."""
    if BENCHMARK_FILE.exists():
        df = load_local_ohlcv(BENCHMARK_TICKER, data_dir)
        if not df.empty:
            return df["close"].rename("benchmark_close")
    # Fallback: fetch via yfinance
    try:
        import yfinance as yf
        bm = yf.download(BENCHMARK_TICKER, start="2015-01-01", auto_adjust=True, progress=False)
        if isinstance(bm.columns, pd.MultiIndex):
            bm.columns = bm.columns.get_level_values(0)
        return bm["Close"].rename("benchmark_close")
    except Exception as e:
        print(f"WARNING: Could not fetch benchmark {BENCHMARK_TICKER}: {e}")
...
def train(panel: pd.DataFrame, benchmark_close: pd.Series,
    ...
    # When running parallel Optuna trials (n_jobs>1), divide cores evenly so
    # n_jobs × num_threads ≈ total CPU cores. Avoids oversubscription.
    import os as _os
    ...
    if n_trials > 0:
        ...
        study_name = f"nse_local_hpo_{mode}"
        ...
        # Cache key: train_end + test_end + feat_hash (MD5 of feature column names).
        # Historical fold data is identical across retrains when features unchanged —
        # as_of was causing full rebuild every retrain. feat_hash invalidates only
        # when the feature set actually changes. Both momentum and reversal get their
        # own cache directory so mode-filtered panels don't collide.
        ...
        _TR_CACHE_COLS = list(pre_selected) + ["cs_rank_20d"]
        ...
        def _fold_cache_path(fold_id: int) -> "Path":
            # _v2: train slices now carry cs_rank_20d (the HPO label) instead of
            # cs_rank_composite — unversioned files lack the column and must miss.
    ...
    with _train_perf.stage("[5/6] Final model training"):
        ...
        _labelled_mask  = full_grp["cs_rank_20d"].notna()
...
def score_and_rank(panel: pd.DataFrame, ensemble, final_features: List[str],
    ...
    shap_img_path = REPORTS_DIR / "shap_global_nse_local.png"
...
def save_outputs(result: dict, panel: pd.DataFrame,
    ...
    try:
        ...
        html_path = rg.generate(REPORTS_DIR / "report_nse_local.html")
